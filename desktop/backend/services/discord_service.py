...
class DiscordService:
        ...
        async def _fetch():
            try:
                ...
                for g in self._client.guilds:
                    results.append({"id": str(g.id), "name": g.name})
    ...
    def get_guild_details(
        ...
        *,
        ...
        """Fetch channels and roles for a specific guild."""
        if not self.is_running:
            if on_error:
                on_error("Bot is not connected.")
            return
        ...
        async def _fetch():
            try:
                g = self._client.get_guild(int(guild_id))
                if not g:
                    g = await self._client.fetch_guild(int(guild_id))
                ...
                for r in g.roles:
                    ...
                    hex_color = f"#{r.color.value:06x}" if r.color.value else ""
                    roles.append({"id": str(r.id), "name": r.name, "color": hex_color})
                roles.sort(key=lambda r: r["name"].lower())

                if on_result:
                    on_result({"channels": channels, "roles": roles})
            except Exception as exc:
                if on_error:
                    on_error(str(exc))
